...
[ $PWM = "yes" ] && echo "CONFIG_PANEL_SMART_DIMMING=y" >> $KERNELCONFIG
...
if [ $MODEL = "S8" ]; then
	...
	sudo sed -i 's/SRPPH16A004KU/SRPPK02A004KU/g' $WORKDIR/ramdisk/split_img/boot.img-board
fi
	cd $WORKDIR/ramdisk
	./repackimg.sh
echo ""
...
if [ $BUILD950 = "yes" ]; then
	...
	LOGFILE=$OUTPUT/build-s8-pwm.log
	PWM=yes
	FUNC_BUILD_BOOTIMG
	mv -f $WORKDIR/ramdisk/image-new.img $ZIPDIR/tgpkernel/kernels/boot-s8-pwm.img
fi
...
if [ $KEEP = "yes" ]; then
	...
	cp -f $ZIPDIR/tgpkernel/kernels/boot-s8+.img $OUTPUT/boot-s8+.img
	cp -f $ZIPDIR/tgpkernel/kernels/boot-s8.img $OUTPUT/boot-s8-pwm.img
	cp -f $ZIPDIR/tgpkernel/kernels/boot-s8+.img $OUTPUT/boot-s8+-pwm.img
	echo ""
fi
ZIP_DATE=`date +%Y%m%d`
